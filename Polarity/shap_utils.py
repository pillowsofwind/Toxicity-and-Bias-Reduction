...
#             # three scores 
#             regard_scores = [shap_values.values[idx][i][0], shap_values.values[idx][i][1], shap_values.values[idx][i][2]]
#             regard_score = np.linalg.norm(regard_scores)
#             if regard_score>0.5:
#                 words.append(word)
#         nans.append(words)
#     return nans
...
def get_dangerous_words(text):
    ...
    for idx in range(len(text)):
        ...
        for i, word in enumerate(shap_values.data[idx]):
            regard_scores = [shap_values.values[idx][i][0], shap_values.values[idx][i][1], shap_values.values[idx][i][2]]
            score = np.linalg.norm(regard_scores)
